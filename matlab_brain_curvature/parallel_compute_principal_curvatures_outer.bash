#!/bin/bash
# parallel_compute_principal_curvatures_outer.bash -- wrapper to compute native space principal curvatures in parallel (using GNU parellel) over a number of subjects
#
######################### IMPORTANT #############################
...
if [ -z "${SUBJECTS_DIR}" ]; then
    echo "$APPTAG WARNING: Environment variable SUBJECTS_DIR not set. Exiting. Set it and run the script from within that dir."
    exit 1
fi
...
if [ -n "$1" ]; then
    ...
    if [ ! -f "$SUBJECTS_FILE" ]; then
        echo "$APPTAG ERROR: Subjects file '$SUBJECTS_FILE' not found. (This is a text file with 1 subject per line.)"
        exit 1
    fi
    if [ -n "$2" ]; then
        SURFACE="$2"
    else
        echo "$APPTAG ERROR: Must specify surface. Usually surface is 'white' or 'pial'. Exiting."
        echo "$APPTAG Usage: $0 <subjects_file> <surface>"
        echo "$APPTAG  <subjects_file>: a text file with 1 subject per line"
        echo "$APPTAG  <surface>: the freesurfer surface, usually one of 'white' or 'pial'"
        exit 1
    fi
else
    echo "$APPTAG ERROR: Must specify subjects_file and surface. Exiting. (subjects_file is a text file with 1 subject per line.)"
    echo "$APPTAG Usage: $0 <subjects_file> <surface>"
    echo "$APPTAG  <subjects_file>: a text file with 1 subject per line"
    echo "$APPTAG  <surface>: the freesurfer surface, usually one of 'white' or 'pial'"
    exit 1
...
echo "$APPTAG Parallelizing over the ${SUBJECT_COUNT} subjects in file '${SUBJECTS_FILE}'."
...
CARGO_SCRIPT="${EXEC_PATH_OF_THIS_SCRIPT}/parallel_compute_principal_curvatures_inner.bash"
